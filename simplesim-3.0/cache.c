...
cache_add(struct cache_t *cp,	/* cache to access */ // Victim Cache
    ...
    // NEED TO CHANGE THE MISS FUNCTION TO ADD THE BLOCK TO THE CACHE
    // OR DO A IF (!CACHE_PROBE(ADDR)) THEN ADD TO CACHE... DONT GET DATA FROM L2 OR MEM, DATA WILL BE PASSED IN
}
